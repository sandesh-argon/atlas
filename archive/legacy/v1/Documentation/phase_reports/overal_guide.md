...
Vars = theory driven, data driven, none
...
Stratify by Region and Income
Check if any indicators overlap
Sort into master format
...
Vars = Raw, Standardized and First Differences 
...
Train on 70% countries (152 countries, all years)
Validate on held-out 15% countries during training for early stopping
Test on final 15% countries for performance reporting
Cross-validation: 5-fold CV within training countries
Hyperparameter tuning: Grid search for learning rate, dropout, regularization strength
...
Train on 70% countries, all years
Multi-task loss: weighted sum of 8 MSE losses
Weight each metric equally OR by inverse variance (to balance scales)
Early stopping on validation set (15% countries)
Output: Single unified model predicting all 8 metrics
...
8 individual models (H5/PKL format)
1 master integrated model (H5/PKL format)
Weight matrices (CSV)
Relationship graphs (CSV + NetworkX pickle)
Feature metadata (CSV)
Normalization parameters (JSON)
Step 10.2: Documentation
Technical report: Methodology, hyperparameters, performance metrics
Feature dictionary: Description of all selected features
API documentation: How to use predict.py for scenarios
Validation report: Out-of-sample performance, robustness tests
