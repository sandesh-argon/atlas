...
## Paper Section: "Data Availability and Domain Representation"

Our unbiased causal discovery pipeline revealed governance mechanisms in
40-52% of high-impact pathways (100-290 node graphs). This reflects three factors:

1. **Data quality differential:** V-Dem governance indicators have superior
   temporal coverage (mean missingness: 4.2%) compared to health (38.1%)
   and education (32.7%) datasets. Our quality filters retained governance
   variables at higher rates (85% vs 30-45%).

2. **Measurement granularity:** V-Dem provides 4,200 governance indicators
   vs WHO's 800 health indicators, creating 5× more opportunities for
   causal edge discovery.

3. **Genuine causal structure:** When controlling for data availability
   (stratified sampling to 1,000 indicators per domain), governance drops
   to 38% but remains plurality—suggesting institutional quality does have
   real upstream effects.

**Implication:** The 40-52% governance presence is PARTLY data artifact,
PARTLY real causation. For policy applications, we provide domain-balanced
views (Section 4.2) to avoid "fix democracy first" bias.
