#!/usr/bin/env python3
"""
A3 Step 3: Remove Cycles from DAG
...
# Setup logging
LOG_FILE = Path(__file__).parent.parent / 'logs' / 'remove_cycles.log'
logging.basicConfig(
    level=logging.INFO,
...
def load_validated_edges():
    ...
    input_file = Path(__file__).parent.parent / 'outputs' / 'A3_validated_fisher_z_alpha_0.001.pkl'

    with open(input_file, 'rb') as f:
        data = pickle.load(f)

    edges_df = data['validated_edges']
...
def save_final_dag(G, validation_results, input_metadata):
    ...
    output = {
        ...
        'metadata': {
            ...
            'input_edges_pc_stable': input_metadata['validated_edges'],
            'after_cycle_removal': G.number_of_edges(),
            'method': 'PC-Stable (Fisher-Z, alpha=0.001) + cycle removal',
            'alpha': input_metadata['alpha'],
            'reduction_from_input': input_metadata['reduction_pct'],
            'total_reduction_from_granger': (1 - G.number_of_edges()/114274)*100
        }
    }
