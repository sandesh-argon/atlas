#!/usr/bin/env python3
"""
A6 Hierarchical Layering - Main Pipeline
...
def build_combined_graph(df_a4, df_a5):
    """
    Build combined NetworkX directed graph with weighted virtual edges.

    CRITICAL ADDITION 1: Virtual Edge Weight Strategy
    - For each A5 interaction (M1 × M2 → Outcome):
      - M1 → INTERACT: weight = |β1| from A4 (main effect)
      - M2 → INTERACT: weight = |β2| from A4 (main effect)
      - INTERACT → Outcome: weight = |β3| from A5 (interaction effect)
    - Fallback: If β1 or β2 not available, use |β3|/2
    """
    logger.info("=" * 70)
    ...
    logger.info(f"  Added {G.number_of_edges():,} A4 edges")

    # Create A4 edge lookup for main effects
    a4_edge_dict = {}
    for _, row in df_a4.iterrows():
        a4_edge_dict[(row['source'], row['target'])] = abs(row['beta'])

    # Add A5 interaction edges with proper weights
    logger.info("Adding A5 interaction effects with virtual nodes...")
    n_virtual = 0
    n_fallback = 0
    ...
    for _, row in df_a5.iterrows():
        ...
        # Create virtual node
        virtual_node = f"INTERACT_{m1}_X_{m2}_TO_{outcome}"
        n_virtual += 1

        # Get main effects from A4 (if available)
        beta_m1 = a4_edge_dict.get((m1, outcome), None)
        beta_m2 = a4_edge_dict.get((m2, outcome), None)

        # Assign edge weights (CRITICAL ADDITION 1)
        if beta_m1 is not None:
            w1 = beta_m1
        else:
            w1 = abs(beta3) / 2
            n_fallback += 1

        if beta_m2 is not None:
            w2 = beta_m2
        else:
            w2 = abs(beta3) / 2
            n_fallback += 1
        ...
        # Add edges
        G.add_edge(m1, virtual_node, weight=w1, edge_type='interaction_input')
        G.add_edge(m2, virtual_node, weight=w2, edge_type='interaction_input')
        G.add_edge(virtual_node, outcome,
                  weight=w3,
                  beta=beta3,
                  r_squared=row['r_squared'],
                  p_value=row['p_value'],
                  edge_type='interaction_output')

    logger.info(f"  Created {n_virtual:,} virtual interaction nodes")
    logger.info(f"  Used A4 main effects: {(n_virtual * 2 - n_fallback):,} / {n_virtual * 2:,}")
    logger.info(f"  Used fallback weights: {n_fallback:,} / {n_virtual * 2:,} ({n_fallback/(n_virtual*2):.1%})")
