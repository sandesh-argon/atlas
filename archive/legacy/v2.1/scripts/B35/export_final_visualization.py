#!/usr/bin/env python3
"""
B3.5 FINAL EXPORT: Visualization-Ready JSON (V2.1)
===================================================
...
from v21_config import A5_OUTPUT, A6_OUTPUT, B1_OUTPUT, B35_OUTPUT
...
# B3.5 hierarchy (V2.1: use config)
with open(B35_OUTPUT / "B35_semantic_hierarchy.pkl", 'rb') as f:
    hierarchy = pickle.load(f)

# B3.5 SHAP scores (composite)
with open(B35_OUTPUT / "B35_shap_scores.pkl", 'rb') as f:
    composite_scores = pickle.load(f)

# B2.5 raw SHAP scores (V2.1: may be in B35_OUTPUT)
b25_path = B35_OUTPUT / "B25_shap_scores.pkl"
if b25_path.exists():
    with open(b25_path, 'rb') as f:
        raw_shap = pickle.load(f)
else:
    raw_shap = composite_scores  # Fallback to composite

# A5 interactions (moderators) (V2.1: use config)
...
# Indicator labels (V2.1: use B1_OUTPUT)
