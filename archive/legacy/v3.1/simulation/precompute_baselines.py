...
OUTPUT_DIR = V31_ROOT / "data" / "v3_1_baselines"

# Also check phase2 API path
PHASE2_PANEL_PATH = V31_ROOT.parent / "viz" / "phase2" / "api" / "data" / "raw" / "v21_panel_data_for_v3.parquet"


def get_available_countries_from_graphs() -> Set[str]:
    """Get list of countries that have temporal graph data."""
    graphs_dir = V31_ROOT / "data" / "v3_1_temporal_graphs" / "countries"
...
def get_available_years_for_country(country: str) -> Set[int]:
    ...
    country_dir = V31_ROOT / "data" / "v3_1_temporal_graphs" / "countries" / country
...
def precompute_baselines(
    ...
    if not panel_path.exists():
        panel_path = PHASE2_PANEL_PATH
    if not panel_path.exists():
        raise FileNotFoundError(f"Panel data not found at {PANEL_PATH} or {PHASE2_PANEL_PATH}")
